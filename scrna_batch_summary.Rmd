...
# in_dir <- "~/rnaseq_qc_report/test_data/B002/rna/B002-P1/labeled_h5/"
...
all_hto_json <- list.files(path = in_dir2, pattern = "hto_processing_metrics.json$", full.names = TRUE, recursive = TRUE)
all_split_json <- list.files(path = in_dir2, pattern = "split_h5_metrics.json$", full.names = TRUE, recursive = TRUE)
...
qc_stacked_barplot_mod(all_meta,
  ...
  ggtitle(sprintf("%s: HTO Category Counts per Well", pools[i]))
...
#### UMAP  
Sample UMAP colored by cell type, 

<div class="superbigimage">
```{r umap_test, fig.height = 4, fig.width = 28, results = "asis"}
...
sample_pool <- unique(meta$pbmc_sample_pool)
...
for(i in seq_along(sample_pool)){
# for(i in 1:3){
  df <- as.data.frame(meta[pbmc_sample_pool ==  sample_pool[i],])
  sid_pool <-   paste0(df$pbmc_sample_id," (",df$pool_id,")")
  
  # cell labels
  g1 <- ggplot(df, aes(umap_1, umap_2)) + 
    geom_point(alpha = 1, size = point_size, aes(color = seurat_pbmc_type)) + 
    ggtitle(paste0(sid_pool,": Cell Labels"))+
    scale_color_manual(values = cell_type_palette$cell_color, labels = cell_type_palette$seurat3_pbmc_type)+
    # scale_color_varibow()+
  ...
  # labeling scores
  g2 <- ggplot(df, aes(umap_1, umap_2)) + 
    geom_point(alpha = 1, size = point_size, aes(color = seurat_pbmc_type_score)) + 
    ggtitle(paste0(sid_pool,": Label Scores"))+
    # facet_wrap(~paste0(pbmc_sample_id,"\n",pool_id), ncol = 1) +
    # scale_color_gradient2(low = "red", mid = "yellow", high ="green3", midpoint = 0.7) +
  ...
  g3 <- ggplot(df, aes(umap_1, umap_2)) + 
    geom_point(alpha = 1, size = point_size, aes(color = fct_mito_umi)) + 
    ggtitle(paste0(sid_pool,": Fraction Mitochondrial UMIs"))+
    # facet_wrap(~paste0(pbmc_sample_id,"\n",pool_id), ncol = 1) +
    # scale_color_gradient2(low = "green3", mid = "yellow", high ="red", midpoint = 0.2) +
    scale_color_gradientn(limits = c(0,1),colours = c("blue", "green3","yellow","red"), 
                          breaks = c(0,0.25,0.5,0.75,1)) +
    # scale_color_varibow() +
    theme_bw()
  ...
  # N Genes 
  g4 <- ggplot(df, aes(umap_1, umap_2)) + 
    geom_point(alpha = 1, size = point_size, aes(color = n_genes)) + 
    ggtitle(paste0(sid_pool,": Gene Counts"))+
    # scale_color_gradient2(low = "blue", mid = "green", high ="red", midpoint = 2000,
    #                       limits = c(min_genes,max_genes), breaks =c(200,2000,4000,6000,8000)) +
    # 
  ...
  # N UMIs 
  g5 <- ggplot(df, aes(umap_1, umap_2)) + 
    geom_point(alpha = 1, size = point_size, aes(color = n_umis)) + 
    ggtitle(paste0(sid_pool,": UMI Counts"))+
    # scale_color_gradient2(low = "blue", mid = "green", high ="red", midpoint = 2000,
    #                       limits = c(min_genes,max_genes), breaks =c(200,2000,4000,6000,8000)) +
    # 
    ...
    theme_bw()
  # plotly::ggplotly(g5)
  aligned_plots <- cowplot::align_plots(g1, g2, g3, g4, g5, align = "hv", axis = "tblr")  # uniform plot sizing regardless of legend size
  print(cowplot::plot_grid(aligned_plots[[1]],
                           aligned_plots[[2]],
                           aligned_plots[[3]],
                           aligned_plots[[4]],
                           aligned_plots[[5]],
...
```{r umaps, fig.width = 10, fig.height=fig_height_samples_4col}
# ggplot(meta, aes(umap_1, umap_2)) + 
#   geom_point(alpha = 0.7, aes(color = seurat_pbmc_type)) + 
#   facet_wrap(~paste0(pbmc_sample_id,"\n",pool_id), ncol = 4) +
#   theme_bw()
```  
...
<!-- #### Fraction Mitochondrial UMI per Cell    -->

```{r fct_mito_umap, fig.width = 10, fig.height=fig_height_samples_4col} 
# ggplot(meta, aes(umap_1, umap_2)) + 
#   geom_point(alpha = 0.6, aes(color = fct_mito_umi)) + 
#   facet_wrap(~paste0(pbmc_sample_id,"\n",pool_id), ncol = 4) +
#   scale_color_gradient2(low = "green3", mid = "yellow", high ="red", midpoint = 0.2) +
#   theme_bw()
...
```{r fct_mito_scatter, fig.width = 10, fig.height=6} 
# ggplot(meta, aes(x = seurat_pbmc_type_score, y = fct_mito_umi)) +
#   geom_point(alpha = 0.3, aes(color = seurat_pbmc_type)) +
#   facet_wrap(~seurat_pbmc_type, ncol = 4) +
#   # facet_wrap(~paste0(seurat_pbmc_type,"\n",pool_id), ncol = 4) +
#   theme_bw()
``` 
...
```{r test, fig.width = 10, fig.height=fig_height_samples_4col}
# test <- read_h5_dgCMatrix(all_files[[2]],"matrix", feature_names = "name")
# table(duplicated(rownames(test)))
# which(duplicated(rownames(test)))
# rownames(test)[which(duplicated(rownames(test)))]
# dupnames <- rownames(test)[rownames(test) %in% rownames(test)[which(duplicated(rownames(test)))]]
# test2 <- read_h5_dgCMatrix(all_files[[2]],"matrix", feature_names = "id")
# table(duplicated(rownames(test2)))
# which(duplicated(rownames(test2)))
# rownames(test2)[which(duplicated(rownames(test2)))]
# dupids <- rownames(test2)[rownames(test) %in% rownames(test)[which(duplicated(rownames(test)))]]
# dupids
                 
# which(rownames(test)=="CD4") 
# rownames(test2)[20240]
