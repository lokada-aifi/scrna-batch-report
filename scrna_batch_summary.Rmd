...
all_multiplet <- list.files(path = in_dir, pattern = "multiplet.h5$", full.names = TRUE, recursive = TRUE)
...
all_counts_list <- lapply(all_files[1:5], H5weaver::read_h5_dgCMatrix, target = "matrix", feature_names = "name")
...
table(rownames(umapDF) == colnames(normCounts))
...
umapDF2 <- cbind(umapDF2, markerDF) %>%
           gather(key = gene, value = value, -UMAP_1_merged, -UMAP_2_merged)
