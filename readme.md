...
per subdirectory are as follows:  
  ...
  * `tcr`: Contains the following well level files generated by `02_run_crossplatform_atac_qc.R` 
  from [`tenx-vdj-pipeline`](https://github.com/aifimmunology/tenx-vdj-pipeline)   
    * `VDJ_T_summary.csv`, Summary files produced by multi_output_formatting.sh
    * `Filtered_Contig_Reformated.csv`, contig files produced by multi_output_formatting.sh
  * `bcr`: Contains the following well level files generated by `02_run_crossplatform_atac_qc.R` 
  from [`tenx-vdj-pipeline`](https://github.com/aifimmunology/tenx-vdj-pipeline)   
    * `VDJ_B_summary.csv`, Summary files produced by multi_output_formatting.sh
    * `Filtered_Contig_Reformated.csv`, contig files produced by multi_output_formatting.sh
