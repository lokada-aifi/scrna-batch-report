...
all_hash_fmt <- t(all_hash) %>% 
  ...
  left_join(in_hto_key, by = "hto_barcode") %>% 
  rename(hto_name_final = hto_name) %>% 
...
df_expected_hto$well_id, 
df_expected_hto$hto_name)
df_expected_hto$n_positive_cells[df_expected_hto$n_positive_cells < 5] <- NA

# N Singlet Cells
hto_well_summary <- all_hash_fmt %>% 
  filter(hto_category == "singlet") %>% 
  filter(hto == hto_barcode) %>% # keep only rows where detected barcode matches final assigned barcode
  group_by(well_id, hto_barcode) %>% 
  summarize(n_singlet = n(),
            .groups = "drop") %>% 
  arrange(well_id) 
df_expected_hto <- df_expected_hto %>% 
  left_join(hto_well_summary, by = c("well_id"="well_id", "hto_barcode"="hto_barcode"))
df_expected_hto$n_singlet[df_expected_hto$n_positive_cells <= 5] <- NA
df_expected_hto$n_singlet[df_expected_hto$n_singlet < 1] <- NA
```


##### HTO Positive Cells per Well
```{r hto_positive_matrix}
# Heat Map 
# temp_scale <- scales::rescale(c(0, 5, 1, max(df_expected_hto$n_positive_cells)))
hm_pos <- ggplot(df_expected_hto, aes(hto_name, y=factor(well_id, levels = sort(unique(well_id), decreasing = T)))) +
  geom_tile(aes(fill = n_positive_cells, color = hto_expected), 
            width = 0.7, height = 0.7, size = 1) +
  scale_fill_viridis_c(option = "D", na.value = "grey") +
  scale_color_manual(breaks = c(TRUE, FALSE), values = c("black","grey")) +
...
##### N Singlet Cells per HTO and Well
```{r hto_singlet_matrix}
hm_singlet <-  ggplot(df_expected_hto, aes(x = hto_name, y=factor(well_id, levels = sort(unique(well_id), decreasing = T)))) +
  geom_tile(aes(fill = n_singlet, color = hto_expected), 
            width = 0.7, height = 0.7, size = 1) +
  scale_fill_viridis_c(option = "D", na.value = "grey") +
  scale_color_manual(breaks = c(TRUE, FALSE), values = c("black","grey")) +
  scale_x_discrete(position = "top") +
...
hash_by_well <- all_hash_fmt %>% 
  group_by(pool_id,well_id, hto,sample_pool_hto, hto_name_barcode) %>% 
...
tempwidth <- n_samples*0.5 + n_pools*0.1 + 2
