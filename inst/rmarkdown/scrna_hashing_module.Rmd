...
```{r hto_counts_well, class.output = "superbigimage"}
...
```{r Well Count By HTO Barcode Plot}
...
temp_figwidth <- (1.2*n_wells + 0.5*n_pools + 3)
temp_figheight <- ceiling(n_pools/2)*6 + 0.4
make_subchunk(g, subchunk_name = "hto_count_subchunk", 
              chunk_opt_list = list(fig.height = temp_figheight, fig.width = temp_figwidth, warning = FALSE), 
              quiet_knit = TRUE)
...
```{r Well Fraction By HTO Barcode Plot}
