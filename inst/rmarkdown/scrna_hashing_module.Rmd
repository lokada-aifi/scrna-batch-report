...
# all_hash_fmt <- all_hash_fmt[well_no_hash, on = c(well_id = "well_id", hto="hto")] 
...
```{r hto_positive_matrix, results="as-is"}
...
hm_pos <- ggplot(df_expected_hto, aes(hto_name, y=factor(well_id, levels = sort(unique(well_id), decreasing = T)))) +
  ...
  scale_fill_viridis_c(option = "D", na.value = "grey") +
    scale_color_manual(breaks = c("Not Used", "Detected","Missing"), values = c("grey","black","red")) +
...
temp_figwidth <- length(unique(df_expected_hto$hto_name))*0.25 + 2
batchreporter::make_subchunk(hm_pos, "hto_positive_matrix_subchunk", quiet_knit = T, 
                             chunk_opt_list = list(height = temp_figheight, width = temp_figwidth))
...
temp_figwidth <- 1.2*n_wells + 0.5*n_pools + 3.5
temp_figheight <- ceiling(n_pools/2)*6 + 0.4
