...
if(is.null(params$in_dir)) {
  batch <- "X002"
  rna_in_dir <- system.file("extdata/X002", package = "batchreporter")
  ...
  # in_config <- "~/Packages/scrna-batch-report/inst/extdata/"
...
if(is.null(params$mc_mb_limit)){
  par_mb_limit <- 500  # Default futures setting, too low for most batches
} else {
...
cnfg <- load_config(in_config = in_config)
...
meta_cols_keep <- c("barcodes", "batch_id", "cell_name", "cell_uuid", "chip_id", 
                    "hto_barcode", "hto_category", "n_genes", "n_mito_umis", "n_reads", 
                    "n_umis", "original_barcodes", sample_column_name, "pool_id", 
                    cell_label_column_name, cell_label_score_column_name, "well_id")
meta_list <- future_lapply(meta_list, function(x){x[, meta_cols_keep]})
...
multiplet_meta_cols_keep <- c("barcodes", "batch_id", "cell_name", "cell_uuid", "chip_id", 
                    "hto_barcode", "hto_category", "n_genes", "n_mito_umis", "n_reads", 
                    "n_umis", "original_barcodes", sample_column_name, "pool_id", "well_id")
multiplet_meta_list <- future_lapply(multiplet_meta_list, function(x){x[, meta_cols_keep]})
...
```{r scrna_seq_child, child =  system.file("rmarkdown/scrna_seq_module.Rmd", package = "batchreporter")}

```
  
### Cell Hashing 

```{r scrna_hashing_child, child = system.file("rmarkdown/scrna_hashing_module.Rmd", package = "batchreporter")} 
...
cell_type_palette <- batchreporter::get_cell_label_palette(cell_label_method = cell_label_method,
...
label_scores_mito <- function(umapDF, cell_label_column_name,
                              mito_binary = "fct_mito_umi_binary"
                              ){
  summary_all <- umapDF %>%
    group_by(!!rlang::parse_expr(cell_label_column_name)) %>%
    summarize(N = n(), .groups = "drop") %>%
    ungroup() %>%
    mutate(pct = N/sum(N)*100) %>%
    mutate(group = "All cells")
 
  label_order <- names(sort(table(summary_all[,cell_label_column_name]), decreasing = TRUE))

  summary_split <- umapDF %>%
    group_by(!!rlang::parse_expr(cell_label_column_name), !!rlang::parse_expr(mito_binary)) %>%
    summarize(N = n(), .groups = "drop") %>%
    ungroup() %>%
  ...
  summary_merge <- rbind(summary_all, summary_split) %>%
    mutate(cell_label = factor(get(cell_label_column_name), levels = label_order))

  # ggplot(summary_merge, aes(factor(cell_label, levels = label_order), pct, fill = group))+
  ...
  ggplot(summary_merge, aes(group, pct, fill = factor(cell_label, levels = label_order))) +
    geom_bar(stat = "identity", position = "stack") +
    xlab("Cell Type Label") +
    ylab("Percent Cell Type within Group") +
    scale_fill_manual(name = "Cell Type", values = cell_type_palette$cell_color, breaks = cell_type_palette$cell_labels)+
    theme(text = element_text(size = 24))
  
}

label_scores_mito(umapDF, cell_label_column_name)
...
Comparing batch bridging control to previous 
...
# g <- ggplot(ctrl_by_type, aes(cell_label_column_name, y = pct)) +
...
labels_order_ctrl <- all_control[,get(cell_label_column_name)][match(all_control$barcodes, colnames(merged_so))]
...
jackstraw_cells_ctrl <- batchreporter::sample_cells(labels_order_ctrl, 100)
