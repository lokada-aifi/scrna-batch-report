...
all_multiplet_h5 <- list.files(path = file.path(rna_in_dir,"multiplet_h5"), pattern = "multiplet.h5$", full.names = TRUE, recursive = TRUE)
all_control_h5 <- list.files(path = file.path(rna_in_dir,"control"), pattern = "_labeled.h5$", full.names = TRUE, recursive = TRUE)
...
```{r scrna_seq_child, child = system.file("rmarkdown/scrna_hashing_module.Rmd", package = "batchreporter")}
```
...
#### Labeling Method: `r {cell_label_method}`
