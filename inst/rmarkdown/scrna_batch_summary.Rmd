...
if(test_format_hook){
  all_labeled_h5 <- list.files(path = in_dir_labeled, pattern = "_labeled.h5$", full.names = TRUE, recursive = TRUE)[1:3]
  all_hto_json <- list.files(path = in_dir_hto, pattern = "hto_processing_metrics.json$", full.names = TRUE, recursive = TRUE)
  all_multiplet_h5 <- list.files(path = in_dir_mult, pattern = "multiplet.h5$", full.names = TRUE, recursive = TRUE)
  all_control_h5 <- list.files(path = in_dir_ctrl, pattern = paste0(control_sample_pattern,".*_labeled.h5$"), full.names = TRUE, recursive = TRUE)[1:3]
} else {
  all_labeled_h5 <- list.files(path = in_dir_labeled, pattern = "_labeled.h5$", full.names = TRUE, recursive = TRUE)
  all_hto_json <- list.files(path = in_dir_hto, pattern = "hto_processing_metrics.json$", full.names = TRUE, recursive = TRUE)
  all_multiplet_h5 <- list.files(path = in_dir_mult, pattern = "multiplet.h5$", full.names = TRUE, recursive = TRUE)
  all_control_h5 <- list.files(path = in_dir_ctrl, pattern = paste0(control_sample_pattern,".*_labeled.h5$"), full.names = TRUE, recursive = TRUE)
}
...
meta_list <- lapply(all_labeled_h5, read_h5_cell_meta)
...
well_list <- lapply(all_labeled_h5, read_h5_well_meta)
...
multiplet_meta_list <- lapply(all_multiplet_h5, read_h5_cell_meta)
...
control_meta_list <- lapply(all_control_h5, read_h5_cell_meta)
...
all_meta[, ":="(pool_id = gsub("C.*", "", pool_id),
            well_id = gsub("-RNA", "", well_id),
            batch_id = gsub("-.*", "", batch_id))]
all_control[, ":="(pool_id = gsub("C.*", "", pool_id),
            well_id = gsub("-RNA", "", well_id),
            batch_id = gsub("-.*", "", batch_id))]
...
```{r}
temp_figwidth <- as.numeric(n_pools*4 + 2.75)
```
...
```{r pct_mito}
meta$fct_mito_group <- cut(meta$fct_mito_umi, breaks = c(-Inf, 0.05, 0.25,0.5,Inf), labels = c("0-0.05","0.1-0.25","0.25-0.5",">0.5"))
...
cumvar_inflection <- cumsum(pc_var/total_var)[inflectionPoint]
cumvar_string <- sprintf(fmt = "%.1f", cumvar_inflection*100)
# p_elbow <- pcElbow +
...
Batch-level UMAP using `r {inflectionPoint}` principal components (`r {cumvar_string}`% explained variance).  
...
```{r umap_quality,  results = "asis", fig.height = 25, fig.width = 30, class.output = "superbigimage"}
...
g1 <- g_base +
  geom_point(alpha = 1, size = point_size, aes(color = seurat_pbmc_type, shape = fct_mito_umi_binary)) +
  ggtitle("Cell Labels")+
...
subplot(gp1, gp2, nrows = 1, margin = 0.05)

```

[Return to Top](#top_cell_labeling) 

#### UMAP for cells with Fraction Mitochondrial UMI`r {fct_mito_umi_threshold}`  
...
```{r cell_labeling_lowmito, fig.height = 10, fig.width = 20}
stm("Plotting Batch UMAP labeling, low fraction mitochondrial reads only")

umapDF_low_mito <- umapDF %>% 
  filter(fct_mito_umi <= fct_mito_umi_threshold) 
umapDF_high_mito <- umapDF %>% 
  filter(fct_mito_umi >= fct_mito_umi_threshold) 
# Gene scaling
# use same gene scaling from plot above

# Cell Types
g_base_mito <- ggplot(umapDF_low_mito, aes(UMAP_1_merged, UMAP_2_merged)) 
...
aligned_plots_mito <- cowplot::align_plots(g1_mito, g2_mito , align = "hv", axis = "tblr")  # uniform plot sizing 

cowplot::plot_grid(aligned_plots_mito[[1]],
                   aligned_plots_mito[[2]],
                   ncol = 2)
```  
...
umapDF2 <- cbind(umapDF2, markerDF) %>% 
  filter(fct_mito_umi < fct_mito_umi_threshold) %>%  # filter to only 
...
all_control <- all_control %>% 
  mutate(control_type = ifelse(batch_id == batch, "new", "old")) 
...
```{r elbowplot_pc_selection_ctrl}
stm("Determining dimensionality via elbow plot method for control")

# Elbow Method
pcElbow <- Seurat::ElbowPlot(object = ctrl_so, reduction = "pca")

# Find inflection point of the elbow curve
inflectionPoint <- inflection::ede(x = pcElbow$data$dims,
                                   y = pcElbow$data$stdev,
                                   index = 1)[, "j2"]
pc_var <- Stdev(ctrl_so, reduction = "pca")^2
total_var <- ctrl_so@reductions$pca@misc$total.variance
cumvar_inflection <- cumsum(pc_var/total_var)[inflectionPoint]
cumvar_string <- sprintf(fmt = "%.1f", cumvar_inflection*100)
# p_elbow <- pcElbow +
#   geom_vline(xintercept = inflectionPoint, col = "red") +
...
pc_embeddings <- ctrl_so@reductions$pca@cell.embeddings
```  
...
ctrl_so_elbow <- Seurat::RunUMAP(ctrl_so,
                                   dims = 1:inflectionPoint,
                                   # umap.method = "umap-learn",
                                   umap.method = "uwot",
                                   seed.use = 3,
...
```{r umap_cleanup_ctrl}
# Get UMAP coordinates
umapDF_ctrl <- ctrl_so_elbow[["umap"]]@cell.embeddings %>%
          as.data.frame() %>%
          dplyr::rename(UMAP_1_merged = UMAP_1, UMAP_2_merged = UMAP_2) %>%
          tibble::rownames_to_column(var = "barcodes")
umapDF_ctrl <- merge(umapDF_ctrl, all_control, by = "barcodes")
rownames(umapDF_ctrl) <- umapDF_ctrl$barcodes
umapDF_ctrl <- umapDF_ctrl[rownames(ctrl_so_elbow@"meta.data"), , drop = F]
...
```{r umap_quality_ctrl,  results = "asis", fig.height = 20, fig.width = 30, class.output = "superbigimage"}
...
# Cell Types
g_base <- ggplot(umapDF_ctrl, aes(UMAP_1_merged, UMAP_2_merged)) 
