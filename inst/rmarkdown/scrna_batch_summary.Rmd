...
#labels_order <- meta$seurat_pbmc_type[meta$barcodes == colnames(merged_so)]
#names(labels_order) <- colnames(merged_so)

#jackstraw_cells <- sample_cells(labels_order, 100)
...
#labels_order_ctrl <- meta$seurat_pbmc_type[meta$barcodes == colnames(merged_so_ctrl)]
#names(labels_order_ctrl) <- colnames(merged_so_ctrl)
...
pc_var_ctrl <- Stdev(ctrl_so, reduction = "pca")^2
total_var_ctrl <- ctrl_so@reductions$pca@misc$total.variance
var_selected_pc_ctrl <- sum(pc_var_ctrl)/total_var_ctrl
cumvar_string_ctrl <- sprintf(fmt = "%.1f", var_selected_pc_ctrl*100)
