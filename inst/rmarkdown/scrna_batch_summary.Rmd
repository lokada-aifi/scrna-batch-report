...
jackstraw_cells <- sample_cells(merged_so$predicted.id, 100)
...
# 
# # Elbow Method
# pcElbow <- Seurat::ElbowPlot(object = merged_so, reduction = "pca", 50)
# 
...
# pc_var <- Stdev(merged_so, reduction = "pca")^2
# total_var <- merged_so@reductions$pca@misc$total.variance
# cumvar_inflection <- cumsum(pc_var/total_var)[nPC]
...
Batch-level UMAP using `r {nPC}` principal components (`r {cumvar_string}`% explained variance) from JackStraw selection.  
...
```{r cell_labeling_mito_celltypes, fig.height = 20, fig.width = 20}
# stm("Plotting Batch UMAP labeling scores, by fraction mitochondrial reads")
...
#   
#   ggplot(summary_merge, aes(seurat_pbmc_type, pct, fill = group))+
...
genes <- c("CD3D","CD3E", "CD3G", "CD4", "CD8A", "HLA-DRA", "CD14", "FCGR3A", "IL3RA", "ITGAX", 
...
lab_order <- sort(table(all_control$seurat_pbmc_type[all_control$control_type=="old"]), decreasing = TRUE)
all_control <- all_control %>% 
...
jackstraw_cells_ctrl <- sample_cells(ctrl_so$predicted.id, 100)
...
stm(sprintf("Selected %s pcs, %s explained variation"), nPC_ctrl, cumvar_string_ctrl)
```
...
Batch-level UMAP using `r {nPC_ctrl}` principal components (`r {cumvar_string_ctrl}`% explained variance).  
...
# Cell Types
g_base <- ggplot(umapDF_ctrl %>% arrange(-control_type), aes(UMAP_1_merged, UMAP_2_merged)) 
