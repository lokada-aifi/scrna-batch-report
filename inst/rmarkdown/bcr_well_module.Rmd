...
```{r, include=FALSE} 
orig_workspace <- ls()
scBCR_seq_well_module_version <- "1.0.0" # 20220125
options(warn = -1)

stm("Starting BCR Well Module")
```
Some codes in module are the modification from Lauren Okada batchreporter scRNA well module codes  

The following metrics summarize the BCR sequencing and alignment by 10x well prior to un-hashing and hash-based cell filtering.  


### Contents 
#### [Detailed Well Summary](#well_seq_summary)
#### [Plots of Well-Level Metrics](#seq_metrics_plots)  
#### [Clonotype Summay](#bcr_clone_summary)
   - [Clonotype Size Distribution](#bcr_clone_size)  
  - [Unique Clonotype](#bcr_unique_clone_type)  
  - [CDR3 Nucleotide Distribution Summary](#bcr_cdr3_nt_distribution)
  - [Clonotype Abundance Summary](#bcr_clone_type_abudance)

 
Check Dependencies  
```{r bcr_dependency_check, include = FALSE}
suppressPackageStartupMessages(library(dplyr))
suppressPackageStartupMessages(library(tidyr))
suppressPackageStartupMessages(library(tibble))
suppressPackageStartupMessages(library(stringr))

assertthat::assert_that(exists("in_bcr"))  

summary_files <- list.files(path = in_bcr, 
                               pattern = "summary.csv", 
                               full.names = TRUE, recursive = TRUE)
assertthat::assert_that(length(summary_files) >0, 
                        msg = sprintf("Did not detect any summary.csv files in input bcr directory %s", in_bcr))
...
assertthat::assert_that(length(contig_files) >0, 
                        msg = sprintf("Did not detect any BCR contig files in input bcr directory %s", in_bcr))

```   


```{r bcr_mummary}
stm("Reading and Merging all bcr summary")
...
```{r bcr_well_plots, results = "asis", fig.height = 3.5}
...
for (i in 1:21){
  df_bcr <- data.table::copy(summary_df)
  
  spec <- bcr_names[i]
  slabel <- vlabels[i]
  df_bcr <- as.data.frame(df_bcr)
  df_bcr$spec_col <- df_bcr %>% dplyr::select (as.name(bcr_names[i]))%>% pull()
  med_val <- median(df_bcr$spec_col)
  cv <- round(sd(df_bcr$spec_col)/mean(df_bcr$spec_col)*100, 2)
  n <- sum(!is.na(df_bcr$spec_col))
  g<-ggplot(df_bcr, aes(WellID, spec_col)) +
  ...
  rm(df_bcr, spec, slabel, med_val, cv, n, g)
...
<a id= "bcr_clone_summary"></a>  


### Clone Type Summary

<a id= "bcr_clone_size"></a>  

#### Clonotype Size Distribution


```{r bcr_clone_size_plots, results = "asis", fig.height = 3.5}
...
<a id= "bcr_unique_clone_type"></a>

#### Unique Clonotype

```{r bcr_clone_unique_plots, results = "asis", fig.height = 3.5}
...
<a id= "bcr_cdr3_nt_distribution"></a>  


#### CDR3 Nucleotide Distribution Summary

```{r bcr_cdr_nt_size_plots, results = "asis", fig.height = 3.5}
...
<a id= "bcr_clone_type_abudance"></a>  

#### Clonotype Abundance Summary

```{r bcr_abundance_plots, results = "asis", fig.height = 3.5,fig.width =12 }
...
[Return to Contents](#bcr_well_top) 

---  

scBCR seq report well module v.`r {scBCR_seq_well_module_version}`, Qiuyu Gong; Lauren Okada  

```{r scbcr_seq_well_cleanup, include = FALSE}
