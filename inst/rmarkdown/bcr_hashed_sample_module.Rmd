<a id="bcr_hashing_top"></a>   

```{r, include=FALSE}  
orig_workspace <- ls()

bcr_sample_module_version <- "1.0.0" #202201024
```
...
#### [scBCR Hash-Based Summary](#hto_scBCR_summary)  
   - [scBCR Well Counts per HTO Barcode Plot](#bcr_well_hto_barcode)   
  - [scBCR Well Fraction per HTO Barcode Plot](#bcr_well_fraction_hto_barcode)  

#### [scBCR Sample Clonotype Summary](#sample_scBCR_summary)  
   - [scBCR Clonotype Size](#bcr_sample_clone_size)   
  - [scBCR Sample Unique Clonotype](#unique_clone_type)
  - [scBCR Sample CDR3 Nucleotide Distribution Summary](#sample_cdr3_nt_distribution)
  - [scBCR Sample Clonotyoe Abundance](#bcr_sample_clonotype_abundance)


```{r include = FALSE}
stm("Starting scBCR Hashing Module (sample-level)")
```  

Check analysis dependencies  
```{r hto_bcr_dependency_check, include = FALSE}  
suppressPackageStartupMessages(library(dplyr))
suppressPackageStartupMessages(library(tidyr))
suppressPackageStartupMessages(library(tibble))
suppressPackageStartupMessages(library(stringr))  
contig_files <- list.files(path = in_bcr,
                               pattern = "Reformatted.csv",
                               full.names = TRUE, recursive = TRUE)
assertthat::assert_that(length(contig_files) >0,
                        msg = sprintf("Did not detect any scBCR contig files in input bcr directory %s", in_bcr))


assertthat::assert_that(exists("in_config"),
                         msg = "Attempting to scBCR quality with hto module. Missing `in_config` variable definition")
assertthat::assert_that(file.exists(in_config),
                         msg = sprintf("Attempting to run scBCR quality with hto module. Input file `in_config` [%s] does not exist", in_config))                        

# check hash folder exists
assertthat::assert_that(dir.exists(in_hto),
                        msg = "Attempting to run scBCR quality with hto module. Input `in_dir` does not contain 'hto' folder")
...
assertthat::assert_that(file.exists(in_key),
                         msg = sprintf("Attempting to scBCR quality with hto module. Input file `in_key` [%s] does not exist", in_key))                        
...
```{r hto_bcr_hash_files}
stm("Reading in hto key for hashed BCR analysis")
...
stm("Reading in hto category metadata for hashed BCR analysis")
...
```{r hto_bcr_merge}
stm("Merging hto category metadata with bcr contig data")
#dt$well_id<-paste0(dt$Batch_ID,"-",dt$Well_ID)
dt$well_id<-paste0(dt$Well_ID)
...
<a id= "hto_scBCR_summary"></a>  


### scBCR Hash-Based Summary

```{r hto_bcr_hash_singlet_summary}
...
chain<-dt %>%    group_by ( pool_id,hto_name, hto_barcode,barcode,well_id,chain) %>%   
        ...
        tally()
IGH<-as.data.frame(chain) %>% filter (chain=="IGH")  %>% select (hto_name,n)
IGK<-as.data.frame(chain) %>% filter (chain=="IGK")%>% select (hto_name,n)
IGL<-as.data.frame(chain) %>% filter (chain=="IGL")  %>% select (hto_name,n)

colnames(IGH)[2]<-"IGH_counts"
colnames(IGK)[2]<-"IGK_counts"
colnames(IGL)[2]<-"IGL_counts"
Sample_HTO_Summary<-left_join(Sample_HTO_Summary,IGH,by=c("hto_name"))
Sample_HTO_Summary<-left_join(Sample_HTO_Summary,IGK,by=c("hto_name"))
Sample_HTO_Summary<-left_join(Sample_HTO_Summary,IGL,by=c("hto_name"))
...
[Return to Contents](#bcr_hashing_top)




<a id= "bcr_well_hto_barcode"></a>  


#### scBCR Well Counts per HTO Barcode Plot

```{r scbcr_Count_By_HTO_Barcode_Plot, results="asis"}
...
cat("\n[Return to Contents](#bcr_hashing_top)", labels = "", sep = "\n")


```

<a id= "bcr_well_fraction_hto_barcode"></a>  

#### scBCR Well Fraction per HTO Barcode Plot


```{r bcr_hto_bar_plot, results="asis"}
...
<a id= "sample_scBCR_summary"></a>  


### scBCR Sample Clonotype Summary

<a id= "bcr_sample_clone_size"></a>  


#### scBCR Clonotype Size

```{r bcr_sample_clone_size_plots, results = "asis", fig.height = 3.5}
...
#### scBCR Sample Unique Clonotype

```{r bcr_sample_clone_unique_plots, results = "asis", fig.height = 3.5}
...
#### scBCR Sample CDR3 Nucleotide Distribution Summary

```{r bcr_sample_cdr_nt_size_plots, results = "asis", fig.height = 3.5}
...
cat("\n[Return to Contents](#bcr_well_top)", labels = "", sep = "\n")
...
<a id= "bcr_sample_clonotype_abundance"></a>  


#### scBCR Sample Clonotyoe Abundance


```{r bcr_sample_abundance_plots, results = "asis", fig.height = 3.5,fig.width =12 }
...
ggplot(abundance, aes(x = n,y = nn,color = sample_hto_pool)) +  
    geom_line()+
    scale_x_log10(breaks = c(1,2,3,4,5,10,20))+
...
scBCR seq report sample module v.`r {bcr_sample_module_version}`, Qiuyu Gong; Lauren Okada  

```{r scbcr_seq_sample_cleanup, include = FALSE}
